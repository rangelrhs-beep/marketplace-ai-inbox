...
from db_models import AiSuggestion, Company, CompanySettings, Integration, ProductCache, PushSubscription, QuestionRecord, User, UserCompanyAccess
...
class PushSubscribeRequest(BaseModel):
    ...
    keys_json: dict[str, Any] = Field(default_factory=dict)
...
def normalize_company_access(role: str, company_id: str | None, company_ids: list[str], access_scope: str | None) -> tuple[str, list[str], str]:
    ...
    if role == "platform_admin":
        ...
        normalized_company_ids = [normalized_company_id]
    primary_company_id = normalized_company_id or (normalized_company_ids[0] if normalized_company_ids else "")
    return primary_company_id, normalized_company_ids, normalized_scope
...
def ensure_database_columns() -> None:
    ...
    with engine.begin() as connection:
        ...
        if engine.dialect.name == "postgresql":
            existing_columns = {
                ...
                for row in connection.execute(
                    text(
                        ...
                        "WHERE table_schema = current_schema() "
                        "AND table_name IN ('ai_suggestions', 'integrations', 'questions', 'company_settings', 'users', 'push_subscriptions')"
                    )
                ).fetchall()
            }
            ...
            for column, column_type in company_settings_columns.items():
            ...
            connection.execute(text("CREATE UNIQUE INDEX IF NOT EXISTS uq_user_company_access_user_company ON user_company_access (user_id, company_id)"))
            connection.execute(text("CREATE INDEX IF NOT EXISTS idx_push_subscriptions_company_user ON push_subscriptions (company_id, user_id)"))
            ...
            sqlite_settings_columns = {
                "ai_general_rules": "TEXT",
                "ai_product_knowledge": "TEXT",
                "ai_allow_web_search": "BOOLEAN NOT NULL DEFAULT 0",
                "ai_absolute_restrictions": "TEXT",
            }
...
def question_to_api(
    ...
    final_answer = question.final_answer or get_final_answer(suggestion)
...
def upsert_portal_answered_ml_question(
    db: Session,
    ...
    question.raw_payload = question_payload
    question.updated_at = datetime.utcnow()
    if question.ai_suggestion:
        db.delete(question.ai_suggestion)
    db.flush()
...
def build_ai_system_prompt(
    *,
    ...
    web_search_enabled = bool(getattr(settings, "ai_allow_web_search", False))
    related_products_context = build_related_products_context(related_products or [])
    related_products_count = len(related_products or [])
    included_absolute_restrictions = bool(absolute_restrictions)
    included_general_rules = bool(general_rules)
    included_product_knowledge = bool(product_knowledge)
    included_related_products = related_products_count > 0
    included_web_search_rules = web_search_enabled
    ...
    print(
        "AI_PROMPT_SECTIONS "
        f"included_absolute_restrictions={included_absolute_restrictions} "
        f"included_general_rules={included_general_rules} "
        f"included_product_knowledge={included_product_knowledge} "
        f"included_related_products={included_related_products} "
        f"included_web_search_rules={included_web_search_rules}",
        flush=True,
    )
    sections = [
        "Você é um vendedor profissional de e-commerce respondendo perguntas no Mercado Livre.",
        "Responda em português do Brasil, com clareza, objetividade e segurança.",
        "Não mencione IA, sistema, API ou instruções internas.",
        "Não há aprendizado automático, auto-treinamento ou expansão autônoma de conhecimento. Use apenas o contexto fornecido.",
        "SECTION 1: ABSOLUTE RESTRICTIONS\n"
        + (absolute_restrictions or "Nenhuma restrição absoluta cadastrada pela empresa."),
        "SECTION 2: GENERAL AI RULES\n"
        + (general_rules or "Nenhuma regra geral cadastrada pela empresa."),
        "SECTION 3: PRODUCT TECHNICAL KNOWLEDGE\n"
        + (product_knowledge or "Nenhum conhecimento técnico de produto cadastrado pela empresa."),
        "SECTION 4: RELATED PRODUCTS / CACHE DATA\n"
        + related_products_context,
    ]
    if web_search_enabled:
        sections.append(
            "SECTION 5: WEB SEARCH RULES\n"
            "A busca complementar na internet está habilitada, mas só pode ser usada quando a base interna, "
            "o conhecimento técnico e os produtos relacionados forem insuficientes. Informações públicas devem "
            "ser tratadas como complementares, nunca podem sobrescrever restrições absolutas ou conhecimento "
            "interno, e nunca devem ser afirmadas como certeza se não forem oficialmente confirmadas."
        )
    else:
        sections.append(
            "SECTION 5: WEB SEARCH RULES\n"
            "A busca complementar na internet está desabilitada. Não use fontes externas nem suponha informações fora da base interna."
        )
    prompt = "\n\n".join(sections)
    print("AI_FINAL_PROMPT_PREVIEW_START", flush=True)
    print(prompt[:3000], flush=True)
    return prompt
...
def extract_ml_question_id_from_notification(payload: Any) -> str | None:
    if not isinstance(payload, dict):
        return None
    question_id = payload.get("question_id")
    if question_id:
        return str(question_id)
    resource = str(payload.get("resource") or payload.get("path") or payload.get("url") or "").strip()
    if not resource:
        return None
    match = re.search(r"/questions/([^/?#]+)", resource)
    if match:
...
def process_mercadolivre_question_notification(db: Session, payload: Any) -> bool:
    ...
    # TODO push: send Web Push on new question webhook to subscribed users for company_id.
    logger.info(
...
class CompanySettingsPayload(BaseModel):
    ...
    tone: str = ""
    custom_prompt: str = ""
    ai_general_rules: str = ""
    ai_product_knowledge: str = ""
    ai_allow_web_search: bool = False
    ai_absolute_restrictions: str = ""
...
def subscribe_push_notifications(payload: PushSubscribeRequest, request: Request, db: Session = Depends(get_db)):
    current_user = get_current_user(request)
    selected_company_id = (payload.company_id or "").strip()
    company_id = selected_company_id or get_current_company_id(request)
    if current_user.role != "platform_admin" and company_id != current_user.company_id:
    ...
    if subscription:
        subscription.keys_json = payload.keys_json or {}
        ...
        db.add(
            PushSubscription(
                ...
                company_id=company_id,
                endpoint=endpoint,
                keys_json=payload.keys_json or {},
            )
        )
    db.commit()
    logger.info("PUSH_SUBSCRIPTION_SAVED user_id=%s company_id=%s", current_user.id, company_id)
    return {"ok": True}


@app.delete("/notifications/push/unsubscribe")
def unsubscribe_push_notifications(payload: PushUnsubscribeRequest, request: Request, db: Session = Depends(get_db)):
    current_user = get_current_user(request)
    selected_company_id = (payload.company_id or "").strip()
    company_id = selected_company_id or get_current_company_id(request)
    if current_user.role != "platform_admin" and company_id != current_user.company_id:
    ...
    db.commit()
    logger.info("PUSH_SUBSCRIPTION_REMOVED user_id=%s company_id=%s deleted=%s", current_user.id, company_id, deleted)
    return {"ok": True}
...
def invite_admin_user(payload: AdminUserInviteRequest, request: Request, db: Session = Depends(get_db)):
    ...
    logger.info("ADMIN_INVITE_START email=%s", email)
    if not email or "@" not in email:
        raise HTTPException(status_code=422, detail="Invalid email.")
    if role not in ALLOWED_ADMIN_LINK_ROLES:
        raise HTTPException(status_code=422, detail="Invalid role.")
...
def generate_openai_rewrite(
    ...
    prompt = (
        f"{system_prompt}\n\n"
        "TAREFA: Reescreva a resposta conforme a instrução do usuário.\n"
...
def generate_openai_initial_suggestion(
    ...
    prompt = (
        f"{system_prompt}\n\n"
        "TAREFA: Crie uma resposta inicial pronta para envio.\n"
...
def debug_ai_prompt(question_id: str, request: Request, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "general_rules": settings.ai_general_rules or "",
        "absolute_restrictions": settings.ai_absolute_restrictions or "",
        "technical_knowledge": settings.ai_product_knowledge or "",
        "related_products_count": len(related_products),
        "web_search_enabled": bool(settings.ai_allow_web_search),
        "final_system_prompt_preview": final_prompt[:5000],
    }
...
def settings_to_api(settings: CompanySettings) -> dict[str, Any]:
    return {
        ...
        "greeting": settings.greeting or "",
        "closing": settings.closing or "",
        "tone": settings.tone or "",
        "custom_prompt": settings.custom_prompt or "",
        "ai_general_rules": settings.ai_general_rules or "",
        "ai_product_knowledge": settings.ai_product_knowledge or "",
        "ai_allow_web_search": bool(settings.ai_allow_web_search),
        "ai_absolute_restrictions": settings.ai_absolute_restrictions or "",
...
def save_company_settings(payload: CompanySettingsPayload, db: Session, company_id: str | None = None) -> dict[str, Any]:
    ...
    settings.ai_general_rules = payload.ai_general_rules.strip()
    settings.ai_product_knowledge = payload.ai_product_knowledge.strip()
    settings.ai_allow_web_search = payload.ai_allow_web_search
    settings.ai_absolute_restrictions = payload.ai_absolute_restrictions.strip()
